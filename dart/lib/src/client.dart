...
class EmojiValidationException implements Exception {
  ...
  EmojiValidationException(this.errors);

  @override
  String toString() => 'EmojiValidationException: ${errors.join("; ")}';
}
...
/// Emoji Lookup API client.
///
/// For more information, visit: https://apiverve.com/marketplace/emoji?utm_source=dart&utm_medium=readme
///
/// Parameters:
/// * [emoji] (required) - The emoji for which you want to get the text representation (e.g., 🥳)
class EmojiClient {
  final String apiKey;
  ...
  static final Map<String, ValidationRule> _validationRules = <String, ValidationRule>{
    'emoji': ValidationRule(type: 'string', required: true),
  };

  EmojiClient(this.apiKey, {
    this.baseUrl = 'https://api.apiverve.com/v1/emoji',
    http.Client? httpClient,
  }) : _httpClient = httpClient ?? http.Client();

  /// Validates parameters against defined rules.
  /// Throws [EmojiValidationException] if validation fails.
  void _validateParams(Map<String, dynamic> params) {
    ...
    if (errors.isNotEmpty) {
      throw EmojiValidationException(errors);
    }
  }

  /// Execute a request to the Emoji Lookup API.
  ///
  /// Parameters are validated before sending the request.
  Future<EmojiResponse> execute(Map<String, dynamic> params) async {
    // Validate parameters
    _validateParams(params);
    if (apiKey.isEmpty) {
      throw EmojiException('API key is required. Get your API key at: https://apiverve.com');
    }
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        throw EmojiException('Invalid API key');
      } else if (response.statusCode == 404) {
        throw EmojiException('Resource not found');
      } else {
        throw EmojiException('API error: ${response.statusCode}');
      }
    } catch (e) {
      if (e is EmojiException) rethrow;
      throw EmojiException('Request failed: $e');
    }
  }
...
/// Exception thrown by the Emoji Lookup API client.
class EmojiException implements Exception {
  final String message;

  EmojiException(this.message);

  @override
  String toString() => 'EmojiException: $message';
}
